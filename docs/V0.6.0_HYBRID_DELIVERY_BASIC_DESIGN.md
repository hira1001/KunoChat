...
- Provider publish/subscribe tokens must be short-lived and scoped to a single device mailbox or send operation.
- Relay/store envelopes must include sender authentication, not only ciphertext.
...
7. Add relay proof behind env flags with scoped provider tokens.
8. Add store-and-forward proof behind env flags.
9. Add encrypted file fallback proof.
10. Enable per-build provider configuration.
11. Run Win/Mac end-to-end tests.
